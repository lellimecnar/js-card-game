...
var Card = function () {
	...
	_createClass(Card, [{
		...
		value: function isSameSuit(card) {
			return Card.compare(this, card, function (_ref) {
				var sameSuit = _ref.sameSuit;
				return sameSuit;
			});
		}
	}, {
		...
		value: function isSameSuitGroup(card) {
			return Card.compare(this, card, function (_ref2) {
				var sameSuitGroup = _ref2.sameSuitGroup;
		...
		value: function isDiff(card, _diff) {
			...
			return Card.compare(this, card, function (_ref3) {
				var numberDiff = _ref3.numberDiff;
		...
		value: function toString() {
			return 'Card {' + _getSuitName.call(this) + ', ' + _getNumberName.call(this) + '}';
		}
	}]);
...
function _logCards() {
	console.log(_getCards.call(this).map(function (card) {
		return [_Card._getSuit.call(card).key, _Card._getNumber.call(card).key];
	}));
}
...
var CardSet = function () {
	...
	_createClass(CardSet, [{
		...
		key: 'sort',
		value: function sort() {
			var numberLength = _getConfig.call(this).numbers.length;

			_getCards.call(this).sort(function (a, b) {
				var aSort = _Card._getSuit.call(a).value.index * numberLength + _Card._getNumber.call(a).value.index;
				var bSort = _Card._getSuit.call(b).value.index * numberLength + _Card._getNumber.call(b).value.index;
